@extends('layouts.app')
@section('content')
<x-common.page-breadcrumb pageTitle="Edit Service Area" />
@if ($errors->any())
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800">
        <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">Edit Service Area</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui data <strong class="text-gray-700 dark:text-gray-200">{{ $serviceArea->name }}</strong>.</p>
    </div>
    <div class="p-6">
        <form action="{{ route('service-areas.update', $serviceArea) }}" method="POST" id="form-edit">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $serviceArea->name) }}" placeholder="Contoh: Service Area Pekanbaru Utara"
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" id="sa_code" name="sa_code" value="{{ old('sa_code', $serviceArea->sa_code) }}" placeholder="Contoh: PKU-01"
                ...
                <div class="flex flex-col gap-1.5 md:col-span-2" x-data="{ selected: '{{ old('datel_id', $serviceArea->datel_id) }}' }">
                    ...
                    <div class="relative">
                        <select id="datel_id" name="datel_id" x-model="selected"
                            ...
                            @foreach($datels as $datel)
                                <option value="{{ $datel->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('datel_id', $serviceArea->datel_id) == $datel->id ? 'selected' : '' }}>

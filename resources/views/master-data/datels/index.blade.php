@extends('layouts.app')
@section('content')
<x-common.page-breadcrumb pageTitle="Datel" />
@if (session('success'))
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Datel</p></th>
                    <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Branch</p></th>
                    <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Witel</p></th>
            ...
            <tbody>
                @forelse($datels as $datel)
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                        <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $datels->firstItem() + $loop->index }}</p></td>
                        <td class="px-5 py-4 sm:px-6"><p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $datel->name }}</p></td>
                        <td class="px-5 py-4 sm:px-6">
                            <span class="inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-theme-xs font-medium text-blue-700 dark:bg-blue-500/15 dark:text-blue-400">{{ $datel->branch?->name ?? '-' }}</span>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <span class="inline-flex rounded-full bg-purple-50 px-2 py-0.5 text-theme-xs font-medium text-purple-700 dark:bg-purple-500/15 dark:text-purple-400">{{ $datel->branch?->witel?->name ?? '-' }}</span>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('datels.edit', $datel) }}" class="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-100 transition-colors dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-400">
                                ...
                                <button type="button" @click="$dispatch('open-delete-modal', { id: {{ $datel->id }}, name: '{{ addslashes($datel->name) }}' })"
                ...
                @empty
                    <tr><td colspan="5" class="px-5 py-10 text-center text-sm text-gray-400 dark:text-gray-500">Belum ada data Datel.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    @if($datels->hasPages())
    <div class="border-t border-gray-100 dark:border-gray-800 px-5 py-3">{{ $datels->links() }}</div>
    @endif
</div>

@include('master-data._partials.delete-modal', ['route' => 'datels'])
@endsection

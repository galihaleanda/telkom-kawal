@extends('layouts.app')
@section('content')
<x-common.page-breadcrumb pageTitle="Edit PIC" />
@if ($errors->any())
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800">
        <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">Edit PIC</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui data <strong class="text-gray-700 dark:text-gray-200">{{ $pic->name }}</strong>.</p>
    </div>
    <div class="p-6">
        <form action="{{ route('pics.update', $pic) }}" method="POST" id="form-edit">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $pic->name) }}" placeholder="Contoh: Budi Santoso"
                ...
                <div class="flex flex-col gap-1.5" x-data="{ selected: '{{ old('service_area_id', $pic->service_area_id) }}' }">
                    ...
                    <div class="relative">
                        <select id="service_area_id" name="service_area_id" x-model="selected"
                            ...
                            @foreach($serviceAreas as $sa)
                                <option value="{{ $sa->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('service_area_id', $pic->service_area_id) == $sa->id ? 'selected' : '' }}>

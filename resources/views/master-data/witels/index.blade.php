...
<x-common.page-breadcrumb pageTitle="Witel" />
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <th class="px-5 py-3 text-left sm:px-6">
                        <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Witel</p>
                    </th>
                    <th class="px-5 py-3 text-left sm:px-6">
                        <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Jumlah Branch</p>
                    </th>
                ...
                @forelse($witels as $witel)
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                        <td class="px-5 py-4 sm:px-6">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $witels->firstItem() + $loop->index }}</p>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $witel->name }}</p>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <span class="inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-theme-xs font-medium text-blue-700 dark:bg-blue-500/15 dark:text-blue-400">
                                {{ $witel->branches_count ?? $witel->branches()->count() }}
                            </span>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('witels.edit', $witel) }}"
                                ...
                                <button type="button"
                                    @click="$dispatch('open-delete-modal', { id: {{ $witel->id }}, name: '{{ addslashes($witel->name) }}' })"
...
{{-- Delete Modal --}}
@include('master-data._partials.delete-modal', ['route' => 'witels'])

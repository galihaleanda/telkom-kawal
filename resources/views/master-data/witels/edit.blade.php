@extends('layouts.app')
@section('content')
<x-common.page-breadcrumb pageTitle="Edit Witel" />
@if ($errors->any())
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800">
        <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">Edit Witel</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui data <strong class="text-gray-700 dark:text-gray-200">{{ $witel->name }}</strong>.</p>
    </div>
    <div class="p-6">
        <form action="{{ route('witels.update', $witel) }}" method="POST" id="form-edit">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div class="flex flex-col gap-1.5 md:col-span-2">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $witel->name) }}"

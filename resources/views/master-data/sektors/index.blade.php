@extends('layouts.app')
@section('content')
<x-common.page-breadcrumb pageTitle="Sektor" />
@if (session('success'))
...
<div class="mb-4 flex items-center justify-between">
    <div></div>
    <a href="{{ route('sektors.create') }}"><x-ui.button size="sm" variant="primary">+ Tambah Sektor</x-ui.button></a>
</div>
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Sektor</p></th>
            ...
            <tbody>
                @forelse($sektors as $sektor)
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                        <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $sektors->firstItem() + $loop->index }}</p></td>
                        <td class="px-5 py-4 sm:px-6"><p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $sektor->name }}</p></td>
                        <td class="px-5 py-4 sm:px-6"><span class="inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-theme-xs font-medium text-blue-700 dark:bg-blue-500/15 dark:text-blue-400">{{ $sektor->serviceArea?->name ?? '-' }}</span></td>
                        <td class="px-5 py-4 sm:px-6"><span class="inline-flex rounded-full bg-purple-50 px-2 py-0.5 text-theme-xs font-medium text-purple-700 dark:bg-purple-500/15 dark:text-purple-400">{{ $sektor->serviceArea?->datel?->name ?? '-' }}</span></td>
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('sektors.edit', $sektor) }}" class="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-100 transition-colors dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-400">
                                ...
                                <button type="button" @click="$dispatch('open-delete-modal', { id: {{ $sektor->id }}, name: '{{ addslashes($sektor->name) }}' })"
                ...
                @empty
                    <tr><td colspan="5" class="px-5 py-10 text-center text-sm text-gray-400 dark:text-gray-500">Belum ada data Sektor.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    @if($sektors->hasPages())
    <div class="border-t border-gray-100 dark:border-gray-800 px-5 py-3">{{ $sektors->links() }}</div>
    @endif
</div>

@include('master-data._partials.delete-modal', ['route' => 'sektors'])
@endsection

@extends('layouts.app')
@section('content')
<x-common.page-breadcrumb pageTitle="Edit Sektor" />
@if ($errors->any())
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800">
        <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">Edit Sektor</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui data <strong class="text-gray-700 dark:text-gray-200">{{ $sektor->name }}</strong>.</p>
    </div>
    <div class="p-6">
        <form action="{{ route('sektors.update', $sektor) }}" method="POST" id="form-edit">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div class="flex flex-col gap-1.5">
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-400">Nama Sektor <span class="text-error-500 ml-0.5">*</span></label>
                    <input type="text" id="name" name="name" value="{{ old('name', $sektor->name) }}" placeholder="Contoh: Sektor A"
                ...
                <div class="flex flex-col gap-1.5" x-data="{ selected: '{{ old('service_area_id', $sektor->service_area_id) }}' }">
                    ...
                    <div class="relative">
                        <select id="service_area_id" name="service_area_id" x-model="selected"
                            ...
                            @foreach($serviceAreas as $sa)
                                <option value="{{ $sa->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('service_area_id', $sektor->service_area_id) == $sa->id ? 'selected' : '' }}>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <a href="{{ route('sektors.index') }}" class="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 transition-colors">Batal</a>
